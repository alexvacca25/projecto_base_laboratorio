 ...
 */
  ...
  Future<List<Course>> fetchCourses(int periodoId) async {
    print(periodoId);
    final response = await http.get(Uri.parse('${Config.baseUrl}/labnodos?id=$periodoId&token=123'));
  ...
  Future<List<Laboratorio>> fetchLaboratorios(int periodoId) async {
    print(periodoId);
    final response = await http.get(Uri.parse('${Config.baseUrl}/laboratorios?id=$periodoId&token=123'));
    if (response.statusCode == 200) {
      List<dynamic> data = json.decode(response.body);
  ...
  Future<List<dynamic>> fetchPeriodos() async {
    final response = await http.get(Uri.parse('${Config.baseUrl}/periodosall?token=123'));
    if (response.statusCode == 200) {
    ...
    final response = await http.get(Uri.parse('${Config.baseUrl}/cursosauto?id=$periodoId&token=123'));
    if (response.statusCode == 200) {
...
Future<DeleteResponse> deleteGeneral(int id) async {
    final response = await http.get(Uri.parse('${Config.baseUrl}/quitar?id=$id&token=123'));
